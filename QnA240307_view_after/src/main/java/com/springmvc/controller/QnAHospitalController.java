...
public class QnAHospitalController {
	...
	public String Updatehospitalqnaget(@ModelAttribute("hospitalupdateqna") Hospitalqna qnanumber,
			@RequestParam("hospitalid") String hospitalid, Model model) {
		 System.out.println("update getMapping");
	    Hospitalqna hospitalqna = qnaService.readHospitalqnaBynumber(hospitalid);
	    model.addAttribute("hospital123", hospitalqna);
	    
	    System.out.println(hospitalqna.getNumber());
	    System.out.println(hospitalqna.getHospitalid() );
	    
	    return "/question/hospitalQnA/hospitalupdateqna";
	}
