	...
	<section class="product_description_area">
		<div class="container">
				...
				<div class="tab-pane fade" id="contact" role="tabpanel" aria-labelledby="contact-tab">
				            ...
				            <div id="comment_list">
				                <c:forEach items="${hospitalqnaList}" var="hospitalqna">
				                    ...
				                    <div class="edit_delete_buttons" style="display:none;">
				                        <a href="#" class="edit_buttons" data-hospitalid="${hospitalqna.hospitalid}">수정하기</a>
			        ...
			        <div class="col-lg-6">
							<div class="review_box">
								<h4>QnA 작성</h4>
							  <form:form modelAttribute="hospitalplus" method="post" action="/QnA240228_view_after/hospitalquestion/add" id="contactForm" novalidate="novalidate">
                <div class="col-md-12">
            ...
            <div id="editFormWrapper" style="display:none;">
                <form:form modelAttribute="hospitalupdateqna" method="post" action="/QnA240228_view_after/hospitalquestion/hospitalupdate" id="contactForm" novalidate="novalidate">
                    <div class="col-md-12">
                        <div class="form-group">
                            <!-- hospitalupdateqna 객체의 number 필드를 바인딩합니다. -->
                            <input type="text"  class="form-control" id="number" name="number" readonly="true" value="${hospital123.number}"/>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <!-- hospitalupdateqna 객체의 email 필드를 바인딩합니다. -->
                            <input type="email"  class="form-control" id="email" name="email" value="${hospital123.email}"/>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <!-- hospitalupdateqna 객체의 date 필드를 바인딩합니다. -->
                            <input type="date"  class="form-control" id="date" name="date" value="${hospital123.date}"/>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <!-- hospitalupdateqna 객체의 content 필드를 바인딩합니다. -->
                            <textarea class="form-control"  name="content" id="content" rows="1">${hospital123.content}</textarea>
                        </div>
                    </div>
                    <div class="col-md-12 text-right">
                        <!-- "등록" 버튼을 클릭하면 hospitalupdateqna 객체를 서버로 전송합니다. -->
 ...
 var list = document.getElementById('comment_list').getElementsByClassName('review_item');
  ...
  function deleteConfirm(hospitalid) {
	    if (confirm("삭제하시겠습니까?") == true) {
	        // 직접 URL을 생성하여 이동합니다.
	        location.href = "/QnA240228_view_after/hospitalquestion/hospitaldelete?hospitalid=" + hospitalid;
	    } else {
	        return;
	    }
	}
  ...
  var editButtons = document.getElementsByClassName('edit_buttons');
    for (var i = 0; i < editButtons.length; i++) {
        editButtons[i].addEventListener('click', function(e) {
            ...
            form.method = 'GET'; // 수정할 데이터를 전달하기 위해 GET 방식 사용
            form.action = '/hospitalquestion/hospitalupdate'; // 수정 처리를 담당하는 컨트롤러 경로

            // hidden 필드 추가 - 수정할 댓글의 ID 전달
            var hospitalIdInput = document.createElement('input');
            hospitalIdInput.type = 'hidden';
            hospitalIdInput.name = 'hospitalid'; // 컨트롤러에서 사용할 파라미터명
