...
public class QnAClassRepositoryImpl implements QnAClassRepository{
	...
	public void createClassqna(Classqna classqna) {
	    ...
	    String SQL = "INSERT INTO classqna (number,content,email,date,classid)" + "VALUES(?,?,?,?,?)";
	    
	    template.update(SQL, classqna.getNumber(), classqna.getContent(), classqna.getEmail(),
	            classqna.getDate(), classqna.getClassid());
	...
	public void updateClassqna(Classqna classqnb) {
	    ...
	    String SQL = "UPDATE classqna SET content=?, email=?, date=?  WHERE classid=?";
	    template.update(SQL, classqnb.getContent(), classqnb.getEmail(), classqnb.getDate(), classqnb.getClassid());
	}
