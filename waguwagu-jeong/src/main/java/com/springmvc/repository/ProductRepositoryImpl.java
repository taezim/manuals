...
public class ProductRepositoryImpl implements ProductRepository
{
	...
	private List<Product> listOfProducts = new ArrayList<Product>();
	 ...
	 public ProductRepositoryImpl() 
	 {
		 ...
		 product.setAccumulatedOrders(1);
		 product.setRating(5);
		 ...
		 product.setAccumulatedOrders(0);
		 product.setRating(0);
		 ...
		 product.setCondition("New");
		 product.setAccumulatedOrders(2);
		 product.setRating(4);
	...
	{
		String SQL = "INSERT INTO product(p_id, p_name, p_unitPrice, p_description, p_category, p_unitsInStock, p_condition, p_acmlOrders, p_rating, p_fileName)" + "VALUES(?,?,?,?,?,?,?,?,?,?)";
		template.update(SQL,pd.getProductId(),pd.getProductName(),pd.getUnitPrice(),pd.getDescription(),pd.getCategory(),pd.getUnitsInStock(),pd.getCondition(),pd.getAccumulatedOrders(),pd.getRating(), pd.getFileName());	
	...
	public void setUpdateProduct(Product pd) {
		...
		{
			String SQL = "UPDATE product SET p_name=?, p_unitPrice=?, p_description=?, p_category=?, p_unitsInStock=?, p_condition=?, p_acmlOrders=?, p_rating=?, p_fileName=? WHERE p_id=?";
			template.update(SQL,pd.getProductName(),pd.getUnitPrice(),pd.getDescription(),pd.getCategory(),pd.getUnitsInStock(),pd.getCondition(),pd.getAccumulatedOrders(),pd.getRating(), pd.getFileName(),pd.getProductId());	
	}
		else if(pd.getFileName()==null) 
		{
			String SQL = "UPDATE product SET p_name=?, p_unitPrice=?, p_description=?, p_category=?, p_unitsInStock=?, p_condition=?, p_acmlOrders=?, p_rating=?, p_fileName=? WHERE p_id=?";
			template.update(SQL,pd.getProductName(),pd.getUnitPrice(),pd.getDescription(),pd.getCategory(),pd.getUnitsInStock(),pd.getCondition(),pd.getAccumulatedOrders(),pd.getRating(), pd.getFileName(),pd.getProductId());	
		}
