...
public class QnAHospitalRepositoryImpl implements QnAHospitalRepository{
	...
	public void createHospitalqna(Hospitalqna hospitalqna) {
		...
		String SQL = "INSERT INTO hospitalqna (number,content,email,date,hospitalid)" + "VALUES(?,?,?,?,?)";
		
		template.update(SQL, hospitalqna.getNumber(),hospitalqna.getContent(),hospitalqna.getEmail(),
				hospitalqna.getDate(),hospitalqna.getHospitalid());
	...
	public void updateHospitalqna(Hospitalqna hospitalqna) {
		 System.out.println("qna update Repository");
		    ...
		    String SQL = "UPDATE hospitalqna SET content=?, email=?, date=?  WHERE hospitalid=?";
		    template.update(SQL, hospitalqna.getContent(), hospitalqna.getEmail(), hospitalqna.getDate(), hospitalqna.getHospitalid());
